...
import manifestRaw from "@/data/gallery-manifest.json"
...
/** Generated by scripts/optimize-images.mjs — responsive renditions + intrinsic size. */
export interface ImageMeta {
  /** Public path prefix, e.g. "/gallery/clicks/12" → `${base}-${w}.avif|webp` */
  base: string
  width: number
  height: number
  widths: number[]
...
const imageManifest = manifestRaw as Record<string, ImageMeta>

export function srcSetFor(meta: ImageMeta, ext: "avif" | "webp"): string {
  return meta.widths.map((w) => `${meta.base}-${w}.${ext} ${w}w`).join(", ")
}

/** Manifest lookup for any original public path (e.g. an entry from `photos`). */
export function imageMetaFor(src?: string): ImageMeta | undefined {
  return src ? imageManifest[src] : undefined
}

/** Largest rendition (lightbox tier). */
export function largestSrc(meta: ImageMeta, ext: "avif" | "webp" = "webp"): string {
  const w = meta.widths[meta.widths.length - 1]
  return `${meta.base}-${w}.${ext}`
}
...
function toPhotoItem(entry: PhotographyJsonEntry | DoodlingJsonEntry, title: string): PhotoItem {
  ...
  const src = entry.image ?? entry.photos?.[0]
  return {
    id: entry.id,
    ...
    src,
    photos: entry.photos,
    imageMeta: src ? imageManifest[src] : undefined,
  }
}
